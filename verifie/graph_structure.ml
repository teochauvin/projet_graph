...
module Make=functor (Dom: Hash)-> struct
  ...
  let rec backtrack = fun graphModif graphOrigine keys-> 
    if not(keys = []) then
      begin 
        Dom.replace graphModif (List.hd keys) (Dom.find graphOrigine (List.hd keys));
        backtrack graphModif graphOrigine (List.tl keys)
      end

  (* Fonction principale d'algo DSATUR Branch and Bound (Backtrack) – MÉTHODE EXACTE *) 
  let dsatureBnB = fun graph ->
    let uB = Dom.length graph in 
    let rec aux = fun graph k visited ->
      if not (is_all_colored graph) then 
        begin
          let graph2 = Dom.copy graph in 
          let nodes_list = Dom.fold (fun id _ -> List.cons id) graph [] in
          ...
          let rec loop = fun i ->
            ...
            if test_color graph id_chosen_vertex color then 
              begin
                ...
                ignore (id_chosen_vertex :: visited);
                ignore (aux graph (k+1) visited);
