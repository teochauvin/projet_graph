module Heap = struct 
  ...
  let down = fun heap_struct x comp ->
    ...
    let rec aux = fun i -> 
      ...
      if comp left current && 2*i+1 <= n then begin
        permut heap_struct i (2*i);
        aux (2*i);
      end

      (* On peut progresser vers la droite et la gauche est non vide *)
      else if 2*i+1 <= n && comp right current then begin
...
let () = 
  ...
  (* Fonction de comparaison sur les degres *)
  ...
  let values = [(2,5);(124,150);(8,12);(95,128);(56,94);(87,132);(9,23);(124,231);(11,23)] in
  ...
  (* Pop *) 
  let (id, dsat, d) = Heap.pop hs comp in 
  Printf.printf "\npoped element : id %d, dsat %d d %d\n" id dsat d;
  ...
  let (dsat, d) = Heap.get_value hs 3 in 
  Printf.printf "\nValue found  dsat %d, d %d\n" dsat d; 

  (* Update la valeur d'une case et donc la structure heap toute entière *)
  Printf.printf "\nflag 7\n"; 

  Heap.update hs 3 58 comp; 
